...
def build_inputs(project: dict) -> tuple[dict, dict, dict]:
    ...
    for fk in ("gf", "1f", "2f", "roof"):
        ...
        floors[fk] = {
            ...
            "int_walls_length": w.get("internal_total_m") or f.get("int_walls_length") or 0.0,
            "int_walls_10cm": w.get("internal_10cm_m") or 0.0,
            "int_walls_20cm": w.get("internal_20cm_m") or 0.0,
...
def _get_item_rate(item_key: str, emirate: str) -> float:
    ...
    return 0.0
    default_rate = BOQ_ITEM_DEFAULT_RATES.get(item_key, 0.0)
    ...
    em_display = "Dubai"
    em_clean = emirate.strip().lower()
    if "abu" in em_clean:
        em_display = "Abu Dhabi"
    elif "sharjah" in em_clean:
        em_display = "Sharjah"
    elif "ajman" in em_clean:
        em_display = "Ajman"
        
    mapping = {
        "foundation_pcc": "Ready Mix C20/25",
        "tie_beam_pcc": "Ready Mix C20/25",
        "foundation_concrete": "Ready Mix C30/37",
        "neck_column_concrete": "Ready Mix C30/37",
        "tie_beam_concrete": "Ready Mix C30/37",
        "slab_on_grade": "Ready Mix C30/37",
        "col_gf_to_1st": "Ready Mix C30/37",
        "col_1st_to_2nd": "Ready Mix C30/37",
        "col_roof": "Ready Mix C30/37",
        "beam_concrete_1st": "Ready Mix C30/37",
        "beam_concrete_2nd": "Ready Mix C30/37",
        "beam_concrete_roof": "Ready Mix C30/37",
        "slab_concrete_1st": "Ready Mix C30/37",
        "slab_concrete_2nd": "Ready Mix C30/37",
        "slab_concrete_roof": "Ready Mix C30/37",
        "parapet_concrete": "Ready Mix C30/37",
        "staircase_1st": "Ready Mix C30/37",
        
        "solid_block_work": "Hollow Block 8\"",
        "parapet_block_work": "Hollow Block 8\"",
        "thermal_block_gf": "Hollow Block 8\"",
        "thermal_block_1f": "Hollow Block 8\"",
        "thermal_block_2f": "Hollow Block 8\"",
        "internal_block_gf": "Hollow Block 6\"",
        "internal_block_1f": "Hollow Block 6\"",
        "internal_block_2f": "Hollow Block 6\"",
        
        "foundation_bitumen": "Bitumen Membrane 4mm",
        "neck_column_bitumen": "Bitumen Membrane 4mm",
        "tie_beam_bitumen": "Bitumen Membrane 4mm",
        "block_work_bitumen": "Bitumen Membrane 4mm",
        "roof_waterproofing": "Bitumen Membrane 4mm",
        "wet_wp_1f": "Bitumen Membrane 4mm",
        "wet_wp_2f": "Bitumen Membrane 4mm",
        "balcony_wp_gf": "Bitumen Membrane 4mm",
        "balcony_wp_1f": "Bitumen Membrane 4mm",
        
        "dry_floor_gf": "Porcelain Tiles 60x60",
        "dry_floor_1f": "Porcelain Tiles 60x60",
        "dry_floor_2f": "Porcelain Tiles 60x60",
        
        "wet_floor_gf": "Ceramic Tiles 60x60",
        "wet_floor_1f": "Ceramic Tiles 60x60",
        "wet_floor_2f": "Ceramic Tiles 60x60",
        "wall_tiles_gf": "Ceramic Tiles 60x60",
        "wall_tiles_1f": "Ceramic Tiles 60x60",
        "wall_tiles_2f": "Ceramic Tiles 60x60",
        
        "int_paint_gf": "Emulsion Paint",
        "int_paint_1f": "Emulsion Paint",
        "int_paint_2f": "Emulsion Paint",
        "external_finishes": "External Paint",
        
        "door_openings": "Wooden Door (Standard)",
        "window_openings": "Aluminum Window",
    }
    
    prefix = mapping.get(item_key)
    if prefix:
        db_name = f"{prefix} ({em_display})"
        try:
            df = safe_query("SELECT rate_aed FROM qto_market_prices WHERE item_name = %s", (db_name,))
            if not df.empty and pd.notnull(df.iloc[0]["rate_aed"]):
                db_rate = float(df.iloc[0]["rate_aed"])
                if "block" in item_key:
                    return round(db_rate * 12.5, 2)
                return db_rate
        except Exception:
            pass
            
    factors = {"dubai": 1.0, "abudhabi": 1.05, "sharjah": 0.95, "ajman": 0.90}
    factor = factors.get(em_clean, 1.0)
    return round(default_rate * factor, 2)
...
def build_boq_dataframe_from_project(project: dict) -> tuple[pd.DataFrame, dict]:
    ...
    def add_section(title: str):
        rows.append({
            "#": "", "Description (English)": f"▌ {title}", "البيان": "",
...
def load_project(path: str = PROJECT_JSON) -> dict:
...
if __name__ == "__main__":
    ...
    p = sys.argv[1] if len(sys.argv) > 1 else PROJECT_JSON
    proj = load_project(p)
    df, meta = build_boq_dataframe_from_project(proj)
    data = df[df["_is_header"] == False]
    print(f"\nComputed {len(data)} items.")
    nonzero = data[data["Quantity"] > 0]
    print(f"Non-zero: {len(nonzero)} / {len(data)}")
    print(df.drop(columns=["_is_header"]).to_string(index=False))
    if meta["estimates"]:
        print("\nESTIMATED (confirm in Schedule Review):")
        for e in meta["estimates"]:
            print(f"  - {e}")
    if meta["needs_input"]:
        print("\nNEEDS INPUT:")
        for n in meta["needs_input"]:
            print(f"  - {n}")
