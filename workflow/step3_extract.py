...
def extract_page(page_arr: np.ndarray, drawing_type: str, page_texts: str, user_id: int = None, previous_warnings: list = None, image_path: str = None, pdf_path: str = None, internal_page_idx: int = None, force_reextract: bool = False) -> dict:
    ...
    if drawing_type in COLUMN_TYPES:
        ...
        async def run_dedicated_extractors():
            ...
            # CRITICAL: We use staggered concurrency (0.5s delay) to speed up extraction by 3x 
            # while avoiding Gemini Free Tier instant rate limits that cause 0s.
            import asyncio
            t1 = asyncio.create_task(_ask_ai_with_retry(img_bytes, arch_p, mgr, force_reextract=force_reextract))
            await asyncio.sleep(0.5)
            t2 = asyncio.create_task(_ask_ai_with_retry(img_bytes, wall_p, mgr, force_reextract=force_reextract))
            await asyncio.sleep(0.5)
            t3 = asyncio.create_task(_ask_ai_with_retry(img_bytes, open_p, mgr, force_reextract=force_reextract))
            
            arch_raw, wall_raw, open_raw = await asyncio.gather(t1, t2, t3)
