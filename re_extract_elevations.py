...
async def re_extract_elevations():
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            for p_id, p in ext_res.items():
                ...
                if dtype in ['elevations', 'sections']:
            ...
            if updated_pages > 0:
                ...
                conn.commit()
                print('DB updated with re-extracted elevations successfully.')
                
                # Check height
                c2 = data.get('confirmed_auto_data', {})
                print("New total_villa_height:", c2.get('total_villa_height'))
            else:
                print('No elevations pages found to re-extract.')

if __name__ == '__main__':
    asyncio.run(re_extract_elevations())
